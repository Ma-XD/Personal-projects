...
public class GUI extends JFrame implements ChessUI {
    ...
    private void initButtons() {
        ...
        backButton.addActionListener(e -> rollBack());
        add(backButton);
    }
    ...
    private void drawPosition(Graphics g) {
        if (from != null) {
            ...
            g2D.drawRect(getPanelX(from.col), getPanelY(from.row), IMAGE_SIZE, IMAGE_SIZE);
    ...
    private synchronized void setPos(Position position) {
    ...
    private FigureName selectFigure = FigureName.QUEEN;

    @Override
    public synchronized FigureName selectFigure(String pos) {
        ...
        container.add(createSelectButton(FigureName.QUEEN, "Queen", selectDialog));
        container.add(createSelectButton(FigureName.ROOK, "Rook", selectDialog));
        container.add(createSelectButton(FigureName.BISHOP, "Bishop", selectDialog));
        container.add(createSelectButton(FigureName.KNIGHT, "Knight", selectDialog));
    ...
    private JButton createSelectButton(FigureName name, String text, JDialog dialog) {
    ...
    @Override
    public synchronized Result testAction() {
        while ((from == null || to == null) && !rollBack && !restart) {
            try {
                wait();
